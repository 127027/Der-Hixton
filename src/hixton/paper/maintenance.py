...
PAPER_TABLES = (
    "paper_execution_audit", "paper_events", "paper_positions", "paper_dust",
    "paper_checkpoints", "paper_soak_symbols", "paper_soak", "paper_audit",
    "paper_strategy_state", "paper_settings", "paper_account",
)


def fresh_start_paper(
    config: ProjectConfig, *, project_root: Path, archive: Path, confirmation: str,
) -> dict[str, object]:
    ...
    if config.paper_starting_cash_usdt != 250 or (
        config.paper_slot_count != 3 or config.paper_target_notional_usdt != 80
    ):
        raise ValueError("fresh start requires 250 USDT and 3x80 slots")
        ...
        with closing(sqlite3.connect(database, timeout=5)) as connection:
            ...
            with connection:
                ...
                details: dict[str, object] = {
                    "decision": "DEC-045", "archive": str(archive), "archive_sha256": digest,
                    "archived_row_counts": counts, "strategy_key": strategy.key,
                    "strategy_version": strategy.version, "starting_cash_usdt": "250.00",
                ...
                connection.execute(
                    "INSERT INTO paper_settings VALUES (1, 3, '80.00', 0, ?)", (moment,),
                )
                connection.execute(
